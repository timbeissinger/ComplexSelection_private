...
```{r}
### genos
setwd("/home/beissinger/Documents/WQS_Prediction/Frequency_change/")
...
rrblupFunction <- function(geno, pheno, map,main){
  ...
  ##remove inds without geno & pheno
    ...
    #run rrBLUP
    result <- mixed.solve(pheno2[,2], Z= as.matrix(geno2[,2:ncol(geno2)]), K=NULL, SE=F, return.Hinv=FALSE,method="ML") #don't use cycle as a covariate!
...
par(mar=c(5.1,5.1,4.1,2.1))
hist(cp_test,xlim=sort(c(-3*cp_result,3*cp_result)),main="Crude Protein",cex.main=2,cex.lab=2)
...
par(mar=c(5.1,5.1,4.1,2.1))
hist(drymat_test,xlim=sort(c(-8*drymat_result,8*drymat_result)),main="Dry Matter",cex.main=2,cex.lab=2)
